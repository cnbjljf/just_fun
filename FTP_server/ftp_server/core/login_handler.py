...
class login_handler_ser( object ):
    """
            ...
            if ac.check_account_exist( username ):
                if ac.check_account_status( username ):
                    if ac.judge_passwd( username, passwd ):
                        ...
                        self.conn.send( bytes( 'true', 'utf8' ) )
                        self.ss.close()
                        ...
                        if tmp.count( username ) > 2:
                            ...
                            self.conn.send( bytes( 'the account had locked', 'utf8' ) )
                            return False
                        self.conn.send( bytes( 'false', 'utf8' ) )

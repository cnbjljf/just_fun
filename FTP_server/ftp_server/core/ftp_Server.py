...
class ftp_handler( object ):
    ...
    def __init__(self):
        ...
        #print('_login_user:',_login_user)
        self.conn=self.lhs.conn
        self.ss=self.lhs.ss
        self.addr=self.lhs.addr
    ...
    def check_quota(self,file_size):
        ...
        #这个user_quota-1的目的在于因为当前宿主目录本身不超过1M，所以减去1后就是当前宿主目录最大能用的容量
            ...
            if user_quota <= int(cur_home_sizep[0])*1024+file_size:
                return False
        ...
        if str( self.conn.recv( 20 ).decode( ) ) == 'ok':
            ...
            if self.action=='cd':
                ...
                if direc.startswith(home_dir) and os.path.isdir(direc):
                        ...
                        os.chdir(direc)
                        result_msg="change directory Sucessfully!!"
                else:

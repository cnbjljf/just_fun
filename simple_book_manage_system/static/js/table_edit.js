...
//        以下三行是把现有的内容提出来，等会放到修改框里面去，
        ...
        var cur_ip = $(id).parent().siblings().children('.ip').text();
...
//        下面几步都是通过替换的方式，把当前要编辑的一行信息替换到编辑框里面去
        $('#in_ip').replaceWith('<input  id="in_ip" type="text" value='+cur_ip +'>');
...
//    这个函数在批量编辑的时候使用
    function mul_edit() {
        ...
        $('input[type="checkbox"]').each(function(index,element) {
            if ($(element).attr('checked') == 'checked') {
                var cur_surname = $(element).parent().siblings().children('.surname').text();
                var cur_lastname = $(element).parent().siblings().children('.last_name').text();
                var cur_email = $(element).parent().siblings().children('.email').text();
                //        下面几步都是通过替换的方式，把当前要编辑的一行信息替换到编辑框里面去

                $(element).parent().siblings().children('.surname').replaceWith('<input class="surname"  type="text" value=' + cur_surname + '>');
                $(element).parent().siblings().children('.last_name').replaceWith('<input class="last_name"  type="text" value=' + cur_lastname + '>');
                $(element).parent().siblings().children('.email').replaceWith('<input class="email"  type="text" value=' + cur_email + '>');
            }
...
//    这个方法用来保存批量编辑的
    function mul_edit_save() {
                var cur_surname = $('#surname').val().trim();
                var cur_lastname = $('#last_name').val().trim();
                var cur_email = $('#email').val().trim();
                var email_match = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/ ;
                //var ip_match = /^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/;
                //var port_match = /^\d+$/
                //var hostname_match = /^\w+\-\w+\-[\d+|\w+|\w+\d+|\d+\w]+\-[\w+\d|\d+\w]+$/
                console.log("hostname match",cur_surname,cur_lastname,cur_email );
                //if(!port_match.test(cur_port) ){
                //    alert("你输入的端口不对！应该输入数字");
                //    flaga=1
                //    return false
                //}
                //if(!ip_match.test(cur_ip)){
                //    alert("你输入的IP不符合规范！");
                //    flaga=1
                //    return false
                //}
                if(!email_match.test(cur_email)){
                    alert("你输入的邮件地址不符规范，请更正！");
                    flaga=1;
                    return false
                }
                if(cur_surname && cur_lastname && cur_email){
                    //$(element).parent().siblings().children('.surname').replaceWith('<div class="surname">'+cur_surname +'<div>');
                    //$(element).parent().siblings().children('.last_name').replaceWith('<div class="last_name">'+cur_lastname +'<div>');
                    //$(element).parent().siblings().children('.email').replaceWith('<div class="email">'+cur_email +'<div>');
...
//                }
    ...
    //        下面几步都是通过替换的方式，把当前要编辑的一行信息替换到编辑框里面去
            ...
            var cur_ip = $(element).parent().siblings().children('.ip').val();
            ...
            $(element).parent().siblings().children('.ip').replaceWith('<div class="ip" >'+cur_ip +'</div>');
...
//    在修改框内提交按钮时调用此方法，主要是对输入内容的判断，如果有内容就把表格里面替换成修改后的，没有输入内容题提示输入
    function checkCon() {
        ...
        var host_ip = $('#in_ip').val();
        console.log(host_ip);
        console.log(ip_match.test(host_ip));
        //alert(port_match.test(host_port) && hostname_match.test(host_name))
        if( port_match.test(host_port) && hostname_match.test(host_name) && ip_match.test(host_ip)){
    ...
    //        删除标记位
            ...
            alert('请输入符合公司规范的主机名/IP且端口必须是数字,');
            return false
        }
    }
...
//                })
    ...
    function check_date() {
        ...
        var date = $('#indate').val();
        console.log(date)
        var date_match=/^[19500000-20170000]/;
        if(pub_name && date_match.test(Number(date))){
            return true
        }else{
            alert('您输入的日期有误，或书籍名没有填写')
            return false
        }
    }